...
for(iii=0;iii<100;iii++){
	...
	for(ch = 0; ch < channel.length; ch++){
		...
		setThreshold((30+(bg*2)),255 ); //threshold is set to 30 + two times the background
	
		run("Create Selection");
		run("Create Mask");
		rename("Mask"+channel[ch]);
		imageCalculator("AND create", "Mask1","Mask"+channel[ch]); //make mask of TUNEL and Protein of interest positive nuclei
	...
	sizes = newArray(30,30);//size is 30 pixels for both channels
	for(ch=0;ch<channel.length;ch++){
		imageCalculator("AND create", "DapiMask","Mask"+channel[ch]);
		rename("Result_Dapi_"+channel[ch]);
		setThreshold(1,255 );
		run("Analyze Particles...", "size="+sizes[ch]+"-Infinity pixel show=Nothing clear display exclude include");
	...
	//let user decide if other cell will be analysed
	
	Dialog.create("Do you want another file");
